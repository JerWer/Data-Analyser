...
from PyQt5.QtWidgets import QFileDialog, QMessageBox, QTableWidgetItem,QAction,QColorDialog
...
# Ui_MainWindow, QMainWindow = loadUiType(r'C:\Users\serjw\Documents\nBox\PythonScripts\SERIS-pythonscripts\executables\All\XRDgui.ui')
from XRDgui import Ui_MainWindow
exedirectory=str(Path(os.path.abspath(__file__)).parent)

LARGE_FONT= ("Verdana", 12)
DATA={}# {"name":[[x original...],[y original...],[x corrected...],[y corrected...],[{"Position":1,"PeakName":'(005)',"Intensity":1,"FWHM":1},...],[linestyle, colorstyle,samplename, answer, linewidthstyle]],"name2":[]}
#takenforplot=[]
listofanswer={}
listoflinestyle={}
listofcolorstyle={}
...
# xrdRefPattDir = r'C:\Users\serjw\Documents\nBox\PythonScripts\SERIS-pythonscripts\executables\All\crystalloData'
xrdRefPattDir = os.path.join(exedirectory,'crystalloData')
...
class XRDapp(QtWidgets.QMainWindow):
    ...
    def closeEvent(self, event):
        global DATA, colorstylelist,Patternsamplenameslist,listofanswer0,samplestakenforplot,peaknamesforplot
        global takenforplot,listofanswer,listoflinestyle,listofcolorstyle,listoflinewidthstyle
...
#                Patternsamplenameslist.append(samplename)
                ...
                tempdat.append(y)#corrected y, set as the original on first importation 
                tempdat.append([])#peak data, list of dictionaries
                tempdat.append(['-',colors[len(DATA.keys())],samplename,samplename,int(2)])

                DATA[samplename]=tempdat
                Patternsamplenameslist.append(samplename)
            elif fileextension =='.rd' or fileextension =='.xrdml':
                pass
            elif fileextension =='.csv':
                # print(filerawdata[0])
                if 'Measurement conditions' in filerawdata[0]: # for SERIS pvk lab xrd
                    # print(filerawdata[22])
                    for j in range(22,len(filerawdata),1):
                        x.append(float(filerawdata[j].split(',')[0]))#assume 2theta
                        y.append(float(filerawdata[j].split(',')[1]))  
                    tempdat.append(x)#original x data 2theta
                    tempdat.append(y)#original y data
...
#                tempdat.append(['-',colorstylelist[len(DATA.keys())],samplename,int(2)])
                ...
                for j in range(len(filerawdata)):
                    if ',' in filerawdata[j]:
                        ...
                        tempdat.append(y)#corrected y, set as the original on first importation 
                        tempdat.append([])#peak data, list of dictionaries
                        tempdat.append(['-',colors[len(DATA.keys())],samplename+str(i),samplename+str(i),int(2)])
...
#%%
    ...
    def Export(self):
        ...
        testdata=['name\tPeakName\tPosition\tPositionQ\tIntensity\tFWHM\tPeakArea\tIntegralBreadth\tCrystSize\n']
        ...
        if samplestakenforplot!=[]:
            ...
            for key in samplestakenforplot:
                for item in DATA[key][4]:
                    testdata.append(key +'\t'+ item["PeakName"]+'\t'+str("%.2f"%item["Position"])+'\t'+str("%.2f"%item["PositionQ"])+'\t'+str("%.2f"%item["Intensity"])+'\t'+str("%.2f"%item["FWHM"])+'\t'+str("%.2f"%item["PeakArea"])+'\t'+str("%.2f"%item["IntBreadth"])+str("%.2f"%item["CrystSize"])+'\n')
...
#%%    
    ...
    def updateXRDgraph(self,a):
        ...
        global listoflinestyle, peaknamesforplot
        ...
        if samplestakenforplot!=[]:
            ...
            for item in samplestakenforplot:
                ...
                if self.ui.checkBox_legend.isChecked():
                    self.XRDgraph.plot(x,y, label=DATA[item][5][3],linestyle=DATA[item][5][0],color=DATA[item][5][1],linewidth=DATA[item][5][4])
    #                    coloridx+=1
                else:
                    self.XRDgraph.plot(x,y,linestyle=DATA[item][5][0],color=DATA[item][5][1],linewidth=DATA[item][5][4])
    #                    coloridx+=1
                ...
                if self.ui.checkBox_showOrig.isChecked():
                    ...
                    if max(y)>maxY:
                        maxY=max(y)
                    if self.ui.checkBox_legend.isChecked():
                        self.XRDgraph.plot(x,y, label=DATA[item][5][3]+'_original',linestyle=DATA[item][5][0],color=DATA[item][5][1],linewidth=DATA[item][5][4])
                        coloridx+=1
                    else:
                        self.XRDgraph.plot(x,y, linestyle=DATA[item][5][0],color=DATA[item][5][1],linewidth=DATA[item][5][4])
                        coloridx+=1
...
#        
    ...
    def populate(self):
        ...
        DATAx=DATA
        listofanswer={}
        sampletotake=samplestakenforplot
        
        listoflinestyle={}
        listofcolorstyle={}
        listoflinewidthstyle={}
        for item in DATAx.keys():
            listoflinestyle[item]=DATAx[item][5][0]
            listofcolorstyle[item]=DATAx[item][5][1]
            listofanswer[item]=DATAx[item][5][3]
            listoflinewidthstyle[item]=str(DATAx[item][5][4])
            
        self.clearLayout(self.ui.gridLayout_4)
        self.ui.scrollArea = QtWidgets.QScrollArea(self.ui.EditLegend)
        ...
        self.ui.verticalLayout_legend.setContentsMargins(0,0,0,0)
        self.ui.verticalLayout_legend.setSpacing(0)
            ...
            for item1 in DATAx.keys(): 
                if item1 not in forbiddenrange:
                    if DATAx[item1][5][2] == itemm:
                        ...
                        listofanswer[item1].setText(DATAx[item1][5][3])
                        ...
                        listofanswer[item1].textChanged.connect(self.UpdateXRDLegMod)
                        ...
                        listoflinestyle[item1].setCurrentText(DATAx[item1][5][0])
                        self.horizontalLayout.addWidget(listoflinestyle[item1])
                        listoflinestyle[item1].currentTextChanged.connect(self.UpdateXRDLegMod)
                        ...
                        listoflinewidthstyle[item1].setValue(DATAx[item1][5][4])
                        self.horizontalLayout.addWidget(listoflinewidthstyle[item1])
                        listoflinewidthstyle[item1].valueChanged.connect(self.UpdateXRDLegMod)
                        
                        self.ui.verticalLayout_legend.addWidget(self.frame)
                        break
                    
                    else:
                        listofanswer[item1]=str(DATAx[item1][5][3])
                        listoflinestyle[item1]=str(DATAx[item1][5][0])
                        listofcolorstyle[item1]=str(DATAx[item1][5][1])
                        listoflinewidthstyle[item1]=str(DATAx[item1][5][4])
        
        self.ui.scrollArea.setWidget(self.ui.scrollAreaWidgetContents)
        self.ui.gridLayout_4.addWidget(self.ui.scrollArea, 0, 0, 1, 1)
    ...
    def UpdateXRDLegMod(self):
        ...
        for item in DATA.keys():
            DATA[item][5][4]=int(leglist[item]) 
