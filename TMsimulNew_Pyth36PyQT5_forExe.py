...
# Ui_TransferMatrixModeling, QMainWindow = loadUiType(r'C:\Users\serjw\Documents\nBox\PythonScripts\SERIS-pythonscripts\executables\All\TMsimulGUI.ui')
from TMsimulGUI import Ui_TransferMatrixModeling
exedirectory=str(Path(os.path.abspath(__file__)).parent)
# print(exedirectory)


file = open(resource_path('spectratxtfiles\AM15G.txt'), encoding='ISO-8859-1')
# file = open(os.path.join(exedirectory,'AM15G.txt'), encoding='ISO-8859-1')
# print(os.path.join(exedirectory,'spectratxtfiles\AM15G.txt'))
# file = open(r'C:\Users\serjw\Documents\nBox\PythonScripts\SERIS-pythonscripts\executables\All\spectratxtfiles\AM15G.txt', encoding='ISO-8859-1')
...
#        print(c_list)
    ...
    def scanlayerthickness(self,layertoscan0,scanrange,generationspectrum,incident_medium_n,incident_angle,output_medium_n,TEorTM):#,window):
        ...
        for thick in scanrange:
            ...
            # print(thick)
            # window.ui.textEdit.append(thick)
            # window.ui.textEdit.repaint()
        return retval

    def scanlayerthickness2D(self,layer1toscan0,layer2toscan0,scanrange1,scanrange2,generationspectrum,incident_medium_n,incident_angle,output_medium_n,TEorTM):#,window):
        ...
        for thick1 in scanrange1:
            ...
            # window.ui.textEdit.append(thick1)
            # window.ui.textEdit.repaint()
            # print(thick1)
            # print("")
            retval=[scanrange2]
            ...
            for thick2 in scanrange2:
                ...
                for ii in range(len(self.structure)+2):
                    retval[ii+1].append(gen[ii])
                # self.ui.textEdit.append(thick2)
...
# stackDir			= r'C:\Users\serjw\Documents\nBox\PythonScripts\SERIS-pythonscripts\executables\All\TMstacks'
# matDir			= r'C:\Users\serjw\Documents\nBox\PythonScripts\SERIS-pythonscripts\executables\All\matdata'
# resDir        = r'C:\Users\serjw\Documents\nBox\PythonScripts\SERIS-pythonscripts\executables\All\results'
# stackDir = os.path.join(exedirectory,'TMstacks')
# matDir = os.path.join(exedirectory,'matdata')
# resDir = os.path.join(exedirectory,'results')
stackDir = resource_path('TMstacks')
matDir = resource_path('matdata')
resDir = resource_path('results')
...
# AM1p5Gc=importAM2(r'C:\Users\serjw\Documents\nBox\PythonScripts\SERIS-pythonscripts\executables\All\spectratxtfiles\AM1.5Gdf.csv')
# AM1p5Gc=importAM2(os.path.join(exedirectory,'spectratxtfiles\AM1.5Gdf.csv'))
AM1p5Gc=importAM2(resource_path('spectratxtfiles\AM1.5Gdf.csv'))
...
class Nkplotwin(QtWidgets.QDialog):
    ...
    def updateNkgraph(self,a):
        ...
        try:
            ...
            self.ui.textEdit.append("valueerror")
            self.ui.textEdit.repaint()
            print("valueerror")
            w=[]
            n=[]
...
class reorderwin(QtWidgets.QDialog):
    def __init__(self,window):
        ...
        self.pushButton_reorder.clicked.connect(lambda: self.validate(window))

        self.widget_layout.addWidget(self.list_widget)
        self.setLayout(self.widget_layout)
    def validate(self,window):
        global MatThickActList,reordered
        ...
        window.populate()
        window.ui.textEdit.append('reordered')
        window.ui.textEdit.repaint()
        self.hide()
...
class TMSimulation(QtWidgets.QMainWindow):
    ...
    def DeleteLayer(self):
        ...
        for item in MatThickActList:
            if not item[0]:
                self.ui.textEdit.append(item[0])
                self.ui.textEdit.repaint()
                print(item[0])
    ...
    def loadstack(self):
        global MatThickActList
        global numberofLayer,exedirectory
        
        # directory=os.path.join(str(Path(os.path.abspath(__file__)).parent.parent),'TMstacks')
        
        # directory=exedirectory
        # directory=r'C:\Users\serjw\Documents\nBox\PythonScripts\SERIS-pythonscripts\executables\All\TMstacks'
        # directory=os.path.join(exedirectory,'TMstacks')
        directory=resource_path('TMstacks')
        ...
        if path!='':
            try:
                with open(path[0],'r') as file:
                    MatThickActList=[]
                    for line in file:
                        if line[:-1].split("\t")[2]=='False' or line[:-1].split("\t")[2]=='0':
                            checkval1='0'
                        elif line[:-1].split("\t")[2]=='True' or line[:-1].split("\t")[2]=='1':
                            checkval1='1'
                        if line[:-1].split("\t")[3]=='False' or line[:-1].split("\t")[3]=='0':
                            checkval2='0'
                        elif line[:-1].split("\t")[3]=='True' or line[:-1].split("\t")[3]=='1':
                            checkval2='1'
    ...
    def savestack(self):
        ...
        try:
            ...
            with open(path[0],'w') as file:
                text=''
                for item in MatThickActList:
                    if str(item[4])=='False' or str(item[4])=='0':
                        checkval='0'
                    elif str(item[4])=='True' or str(item[4])=='1':
                        checkval='1'
                    if str(item[3])=='False' or str(item[3])=='0':
                        checkval2='0'
                    elif str(item[3])=='True' or str(item[3])=='1':
                        checkval2='1'
                    text+=item[1]+'\t'+str(item[2])+'\t'+checkval2+'\t'+checkval+'\n'
                file.write(text[:-1])
    ...
    def simulate1(self):
        ...
        global matnamelist
        global MatThickActList
        self.ui.textEdit.append('simulation started... calculating...')
        self.ui.textEdit.repaint()
        
        #ask path to export
        # directory=os.path.join(str(Path(os.path.abspath(__file__)).parent.parent),'results')
        # directory=r'C:\Users\serjw\Documents\nBox\PythonScripts\SERIS-pythonscripts\executables\All\results'
        # directory=os.path.join(exedirectory,'results')
        directory=resource_path('results')
...
#                self.fig.savefig(f[:-4]+'_withParasitic.png', dpi=300, transparent=False) 
        ...
        if self.ui.checkBox_1D.isChecked():
            self.ui.textEdit.append('1D starting... calculating...')
            self.ui.textEdit.repaint()
            w=simul1D(structure,specttotake, f,self)
            w.show()
        elif self.ui.checkBox_2D.isChecked():
            self.ui.textEdit.append('2D starting... calculating...')
            self.ui.textEdit.repaint()
            w=simul2D(structure,specttotake, f,self)
            w.show()
        else:
            self.ui.textEdit.append('simulation finished')
            self.ui.textEdit.repaint()
            


class simul1D(QtWidgets.QDialog):
    def __init__(self, structure, specttotake, f,window):
        super().__init__()
        global numberofLayer
        global matnamelist
        global MatThickActList
        ...
        self.pushButton_1D.clicked.connect(lambda: self.validate(structure,specttotake,f,window))
        
    def validate(self,structure,specttotake,f,window):
        global numberofLayer
        global matnamelist
        global MatThickActList
        
        # window.ui.textEdit.append('validating')
        # window.ui.textEdit.repaint()
        
        d=structure.scanlayerthickness(self.matlistfor2Dvar.index(self.comboBox_matname1D.currentText())+1,range(int(self.Startthick1.text()),int(self.Endthick1.text())+1,int(self.Stepthick1.text())),specttotake,1,0,1,'s')#,window)
        dinv=[list(x) for x in zip(*d)]
        
        # window.ui.textEdit.append('validating2')
        # window.ui.textEdit.repaint()
        ...
        window.ui.textEdit.append('simulation finished')
        window.ui.textEdit.repaint()
...
class simul2D(QtWidgets.QDialog):
    def __init__(self, structure, specttotake, f,window):
        ...
        self.pushButton_2D.clicked.connect(lambda: self.validate(structure,specttotake,f,window))
    ...
    def validate(self,structure,specttotake,f,window):
        ...
        # window.ui.textEdit.append('simulation validating')
        # window.ui.textEdit.repaint()
        
        d=structure.scanlayerthickness2D(self.matlistfor2Dvar.index(self.comboBox_matname2D1.currentText())+1,self.matlistfor2Dvar.index(self.comboBox_matname2D2.currentText())+1,range(int(self.Startthick1.text()),int(self.Endthick1.text())+1,int(self.Stepthick1.text())),range(int(self.Startthick2.text()),int(self.Endthick2.text())+1,int(self.Stepthick2.text())),specttotake,1,0,1,'s')#,window)
        datatoexporttxt=[]
